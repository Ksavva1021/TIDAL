...
with alive_bar(df.shape[0]) as bar:
    for index, row in df.iterrows():
        ...
        vector_lepton_1 = ROOT.TLorentzVector()
        vector_lepton_1.SetPtEtaPhiM(row['pt_1'], row['eta_1'], row['phi_1'], row['mass_1'])
        cartesian_vector_lepton_1 = ROOT.TLorentzVector(vector_lepton_1.Px(), vector_lepton_1.Py(), vector_lepton_1.Pz(), vector_lepton_1.E())
        lepton_1 = Particle(cartesian_vector_lepton_1, row['pi_charge_1'])
        ...
        if row['decayMode_2'] == 0:
            aco_angle_0_0 = AcoplanarityAngle(ip1,ip2,lepton_1.Fourvector,pi_2.Fourvector,type1="IP",type2="IP")
            dict['aco_angle_0_0'].append(aco_angle_0_0)
        else:
            dict['aco_angle_0_0'].append(-9999.0)

        if row['decayMode_2'] == 1:
            aco_angle_0_1 = AcoplanarityAngle(ip1,pi0_2.Fourvector,lepton_1.Fourvector,pi_2.Fourvector,type1="IP",type2="NP")
            dict['aco_angle_0_1'].append(aco_angle_0_1)
        else:
            dict['aco_angle_0_1'].append(-9999.0)

        if row['decayMode_2'] == 10:
            hads = sortA1(pi_2,pi2_2,pi3_2)
            aco_angle_0_10 = AcoplanarityAngle(ip1,hads[0].Fourvector,lepton_1.Fourvector,hads[1].Fourvector,type1="IP",type2="A1")
